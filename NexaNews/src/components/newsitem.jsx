const Newsitem = ({ title, description, src, url }) => {
  return (
    <div className="card  bg-dark text-dark mb-3 d-inline-block  my-3 mx-3 px-2 py-2 col-md-4 col-sm-6" style={{maxWidth:345}}>
      <img src={src || "https://via.placeholder.com/150"} alt="News"  style={{height:"200px",width:"320px"}}/>
      <h5 className="card title">{title.slice(0,50)}</h5>
      <p className="card text"> {description?description.slice(0,90):"No description available for this news."}</p>
      <a href={url} target="_blank" rel="noopener noreferrer" className="btn btn-primary">Read more</a>
    </div>
  );
};

export default Newsitem;
